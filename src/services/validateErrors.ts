import { inputDelyveryValues, inputsPaymentValues } from "../conts";
...
export const validateErrors = ({ input, setErrors, price }: validateErrorsProps) => {
    ...
    if (input.delivery == inputDelyveryValues.home && input.direction?.length == 0) {
    ...
    if (input.payment == inputsPaymentValues.efective && Number(input.cash) < Number(price)) {
